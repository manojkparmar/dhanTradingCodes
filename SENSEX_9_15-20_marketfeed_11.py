"""
╔══════════════════════════════════════════════════════════════╗
║         Sensex Options Trading Bot — Dhan Broker             ║
║  Strategy: Square-Root Level Breakout with ATM CE/PE Entry   ║
╚══════════════════════════════════════════════════════════════╝

Flow:
  1. Wait for 9:20 AM → fetch 9:15 5-min Sensex SPOT candle close
...
NOTE — BSE / Sensex specifics:
  • Security ID : 1    (Dhan security ID for SENSEX)
  • Exchange    : BSE_FNO  for options orders
  • Index feed  : IDX_I
  • Strike step : 100  (Sensex options in multiples of 100)
  • Lot size    : 10   (verify current with BSE)
"""
...
log_filename = "run_log/" + f"SENSEX_{timestamp}.out"
...
# ── Suppress Windows asyncio WinError 6 shutdown noise ────────────────────────
class _SuppressOverlappedNoise(logging.Filter):
    """
    Drop the 'Cancelling an overlapped future failed / WinError 6' error that
    Windows ProactorEventLoop emits on every daemon-thread teardown.
    This fires on the 'asyncio' logger from whichever thread owns the loop,
    so patching the event loop exception handler is not enough — we must
    filter it at the logging layer instead.
    """
    def filter(self, record: logging.LogRecord) -> bool:
        msg = record.getMessage()
        if "overlapped" in msg.lower() and "winerror 6" in msg.lower():
            return False   # drop it
        if "cancelling an overlapped" in msg.lower():
            return False   # drop it
        return True

logging.getLogger("asyncio").addFilter(_SuppressOverlappedNoise())
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f"logs/snx_bot_{timestamp}.log", encoding="utf-8"),
    ],
)
log = logging.getLogger("SensexBot")
...
# ═════════════════════════════════════════════════════════════════════════════
# CONSTANTS  — Sensex specific
# ═════════════════════════════════════════════════════════════════════════════

SENSEX_SECURITY_ID  = "51"        # Dhan security ID for SENSEX index
SENSEX_INDEX_EXCH   = "IDX_I"    # Exchange segment for index spot quote
SENSEX_FNO_EXCH     = "BSE_FNO"  # Exchange for Sensex F&O orders (BSE, not NSE)
INSTRUMENT_INDEX    = "INDEX"
INSTRUMENT_OPTIDX   = "OPTIDX"

LOT_SIZE              = 20        # Sensex lot size (verify current with BSE)
STRIKE_STEP           = 100       # Sensex options in multiples of 100
POLL_INTERVAL_SEC     = 1
PREMIUM_TARGET_POINTS = 30
ENTRY_BUFFER          = 0
ORDER_TYPE            = "MARKET"
PRODUCT               = "INTRADAY"
SUMMARY               = ""
...
def get_candle_close(hour: int = CANDLE_HOUR, minute: int = CANDLE_MINUTE) -> float:
    """
    Fetch today's 5-min Sensex candle for the given hour:minute and return its close.
    """
    candle_label = f"{hour}:{minute:02d}"
    today_str    = date.today().strftime("%Y-%m-%d")
    log.info(f"Fetching Sensex 5-min candle for {candle_label} | date={today_str} ...")

    resp = dhan.intraday_minute_data(
        security_id      = SENSEX_SECURITY_ID,
        exchange_segment = SENSEX_INDEX_EXCH,
...
def _start_feed(option_security_id: str | None = None):
    """
    Build a MarketFeed subscribed to Sensex spot (security_id="1")
    + optional option, then launch the background thread.

    NOTE: Sensex options trade on BSE_FNO, so option instruments
    use MarketFeed.BSE_FNO (verify the constant name in your dhanhq version).
    """
    global _feed_instance, _feed_thread, _feed_subscribed_ids, _feed_stop_event

    instruments   = [(MarketFeed.IDX, SENSEX_SECURITY_ID, MarketFeed.Quote)]
    if option_security_id:
        # Sensex options are on BSE_FNO; use the correct MarketFeed exchange constant.
        # In dhanhq v2 this is typically MarketFeed.BSE_FNO — verify in your SDK.
        instruments.append(
            (MarketFeed.BSE_FNO, str(option_security_id), MarketFeed.Quote)
        )
...
def get_live_spot() -> float:
    """Return the latest Sensex spot LTP from the background feed."""
    return _read_ltp(SENSEX_SECURITY_ID)


def get_spot_and_option_ltp(security_id: str) -> tuple[float, float]:
    spot = _read_ltp(SENSEX_SECURITY_ID)
    opt  = _read_ltp(str(security_id))
    return spot, opt


# ═════════════════════════════════════════════════════════════════════════════
# OPTION CHAIN HELPER  — Sensex strike step = 100
# ═════════════════════════════════════════════════════════════════════════════

def get_atm_option(spot: float, option_type: str) -> dict:
    sx_all_exp    = dhan.expiry_list(int(SENSEX_SECURITY_ID), "IDX_I")
    sx_latest_exp = sx_all_exp["data"]["data"][0]

    print("\n Working on Sensex's Expiry", sx_latest_exp, ",\n")

    # Sensex strikes in multiples of 100
    ...
    print("\n Sensex ATM Strike & Expiry is: ", atm_strike, option_type, sx_latest_exp, "\n")

    resp = dhan.option_chain(int(SENSEX_SECURITY_ID), "IDX_I", sx_latest_exp)
...
# ═════════════════════════════════════════════════════════════════════════════
# ORDER MANAGEMENT  — NOTE: Sensex orders go to BSE_FNO
# ═════════════════════════════════════════════════════════════════════════════
...
def _place_order(security_id: str, txn_type: str) -> str:
    resp = dhan.place_order(
        ...
        exchange_segment = dhan.BSE_FNO,   # ← BSE for Sensex options
...
def print_levels(levels: dict, close: float, candle_label: str = None):
    ...
    w = 46
    print(f"\n{'═' * w}")
    print(f"  SENSEX  |  {candle_label} Close: {close:,.2f}")
    print(f"{'═' * w}")
...
def run_bot():
    ...
    log.info("══════════════════════════════════════════")
    log.info("  Sensex Options Bot — Starting Up")
